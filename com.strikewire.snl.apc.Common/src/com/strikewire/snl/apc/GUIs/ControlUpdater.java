...
public class ControlUpdater
{
  ...
  private static boolean invokeSetEnabledMethod(final Control control,
                                                boolean enabled)
  {
    return invokeSetBooleanMethod(control, "setEnabled", enabled);
  }
   ...
   * @author kholson
   *         <p>
   *         Initial Javadoc date: Aug 6, 2019
   *         </p>
   *         <p>
   *         Permission Checks:
   *         </p>
   ...
   *         </p>
   */
  private static boolean invokeSetSelectionMethod(final Control control,
                                                  boolean enabled)
  {
    return invokeSetBooleanMethod(control, "setSelection", enabled);
  }
  ...
  /**
   * invokes via reflection a setX method that takes a boolean
   * <ul>
   * <li>setEnabled</li>
   * <li>setSelection
   * <li>
   ...
   *         </p>
   */
  private static boolean invokeSetBooleanMethod(final Control control,
                                                String methodName,
                                                boolean enabled)
  {
    Method method = null;
  ...
  /**
   * On the specified control, invoke the setSelection setting the selection to
   * the specified sel value; shifts to UI thread if necessary; ensures that
   * control is not null or disposed
   * 
   * @param control
   * @param sel
   * @author kholson
   ...
   *         </p>
   */
  public static void setSelectionOnControl(final Control control,
                                           final boolean sel)
  {
    if (control == null || control.isDisposed()) {
      return;
    ...
    if (CommonMessages.currentThreadIsDisplayThread()) {
      invokeSetSelectionMethod(control, sel);
    }
    else {
  ...
  public static void setEditableOnControl(final Control control,
                                          final boolean editable)
  {
    ...
    if (CommonMessages.currentThreadIsDisplayThread()) {
      invokeSetBooleanMethod(control, "setEditable", editable);
    }
    else {
  ...
  /**
   * Method set's both the visibility on the given control, and if the layout
   * data is a GridData, also set's the attached GridData's exclude to the
   * inverse of the visible argument.
   * 
   * @return - true if the call to this method made any changes, false otherwise
   */
  public static boolean setCompleteVisibilityOnGridLayoutControl(Control control,
                                                                 boolean visible)
  {
