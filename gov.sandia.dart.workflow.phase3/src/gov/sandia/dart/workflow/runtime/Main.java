...
public class Main {
	...
	public static void main(final String[] args) throws Throwable {
		LoggingWorkflowMonitor monitor = null;
		try {
			GetOpt options = new GetOpt(args, "kvV", "dhwxpg");
			String[] params = options.params();

			if (options.hasOption("V")) {
			  System.out.println(WorkflowProcess.getWorkflowVersion());
			  System.out.println(WorkflowProcess.getWorkflowInstallLocation());
			}
			...
			if (!workDir.exists()) {
				if (!workDir.mkdirs()) {
					// TODO If we're being invoked by Dakota, then this is a bad situation,
					// since we can't write the results file, so Dakota won't know we've failed.
			...
			monitor = new LoggingWorkflowMonitor(logFile);
			try  {
				...
				if (dakotaMode) {
					DakotaResultsFile.writeFailure(options.params()[2], ex);
				}
				monitor.terminated(ex);
				throw ex;
			}
