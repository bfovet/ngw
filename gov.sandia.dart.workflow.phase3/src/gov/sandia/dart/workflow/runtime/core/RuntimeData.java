...
public class RuntimeData {
	...
	private Set<String> dummies = ConcurrentHashMap.newKeySet();
	...
	public boolean isBreakpoint(String name) {
		return facade != null ?
				facade.isBreakpoint(name) : false;
	}
	
	public boolean isRunThroughBreakpoints() {
		return facade != null ?
			facade.isRunThroughBreakpoints() : false;
	}
	...
	public void breakpointHit(SAWCustomNode node) {
		cancel();
		monitors.forEach(m -> m.breakpointHit(node, this));
	}
	...
	public void status(SAWCustomNode node, String status) {
	...
	boolean isDummy(String name) {
		return dummies.contains(name);
	}
	
	void setDummy(String name) {
		dummies.add(name);
	}

	void clearDummy(String name) {
		dummies.remove(name);
	}
}
