...
public class ClasspathTool {
	...
	public static String generateBashScript() throws IOException {		
		...
		String cubitPath = System.getenv("CUBIT_BASE_PATH");
		if (cubitPath != null)
			builder.append("export CUBIT_BASE_PATH=").append(cubitPath).append("\n");
		...
		builder.append("\n\n");
		builder.append("\"").append(java).append("\"");
		builder.append(" \\\n");
	...
	public static String generateBatchFile() throws IOException {		
		...
		String cubitPath = System.getenv("CUBIT_BASE_PATH");
		if (cubitPath != null)
			builder.append("set CUBIT_BASE_PATH=").append(cubitPath).append("\r\n");
		// TODO System properties substitution
		EmbeddedExecutionEnvironmentVariables.getInstance().
					...
					if (url != null) {
						...
						if (url != null) {
							try {
								...
								// Must be a jar, nothing we can do.
							}
						}
					}
				}
		...
		for (String id: ids) {
			...
			if (bundle != null) {
				...
				if (url != null)
					url = FileLocator.resolve(url);
				if (url == null) {
					// May mean the plugin is jarred-up, or that there is no .classpath file.

					URL url2 = FileLocator.find(bundle, new Path(""), null);
					if (url2 != null) {
						try {
							url2 = FileLocator.resolve(url2);
							File entry = new File(url2.getPath());
							entries.add(entry.getCanonicalPath());
						} catch (IOException ioe ) {
							// It's a jar. Nothing we can do.
						}
					}
