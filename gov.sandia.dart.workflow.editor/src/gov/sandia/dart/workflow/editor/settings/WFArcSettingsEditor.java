...
public class WFArcSettingsEditor extends AbstractSettingsEditor<WFArc> {
	...
	public void createPartControl(IManagedForm mform, IMessageView messageView, MultiControlSelectionProvider selectionProvider, IContextMenuRegistrar ctxMenuReg)
	{
		...
		if (store.getBoolean(ISettingsViewPreferences.DRAW_BORDERS)) {
			toolkit.paintBordersFor(linkingClient);
		}
	...
	public static boolean updateConnectionAppearance(Diagram diagram, IFeatureProvider fp ,Arc node) {
		...
		LineStyle lineStyle = expand ? LineStyle.DASH : read ? LineStyle.DOT : LineStyle.SOLID;
		if (!lineStyle.equals(ga.getLineStyle())) {
			ga.setLineStyle(lineStyle);
			changed = true;
		}
		int lineWidth = expand ? 2 : read ? 2 : 1;
		if (lineWidth != ga.getLineWidth()) {
		...
		if (!color.equals(ga.getForeground())) {
