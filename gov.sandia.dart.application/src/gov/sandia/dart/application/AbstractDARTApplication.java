 ...
 * {@link AbstractDARTWorkbenchWindowAdvisor} have been created for two primary purposes:
 * <ol>
 * <li>Provide a framework for sending events to registered listeners for the various
 * application life-cycle states (see {@link DARTApplicationEvent}).</li>
 * <li>Provide a place for consolidated common behavior for applications and
 * advisors instead of maintaining several unique concrete instances which are largely
 * replications of each other.</li>
 * </ol>
 * 
 * TODO: Might want to consider having this abstract application and the advisors provided
 * in this package extend from the IDEApplication and the advisors related to it.  This
 * suggestion because much of the code in the application and advisor classes that have
 * been created to date looks like it was originally plagiarised from the IDEApplication
...
public abstract class AbstractDARTApplication implements IApplication, IExecutableExtension
{
	/**
    ...
    protected static final String WORKSPACE_CHECK_REFERENCE_BUNDLE_NAME = "org.eclipse.platform"; //$NON-NLS-1$
    ...
    protected static final String WORKSPACE_CHECK_REFERENCE_BUNDLE_NAME_LEGACY = "org.eclipse.core.runtime"; //$NON-NLS-1$
    protected static final String WORKSPACE_CHECK_LEGACY_VERSION_INCREMENTED = "2"; //$NON-NLS-1$   legacy version=1
     ...
     */
    protected static final Integer EXIT_RELAUNCH = new Integer(24);
     ...
     */
    protected static final Integer EXIT_WORKSPACE_LOCKED = new Integer(15);
	...
	{
		DARTApplicationEventDispatch.preNotify(DARTApplicationEvent.APPLICATION_CONSTRUCT);
	}
	
	@Override
	public final Object start(IApplicationContext context) throws Exception
	{
		DARTApplicationEventDispatch.preNotify(DARTApplicationEvent.APPLICATION_START);
		Object ret = doStart(context);
		DARTApplicationEventDispatch.postNotify(DARTApplicationEvent.APPLICATION_START);
		return ret;
	}
	...
	protected Object startHeadedClient(IApplicationContext context) throws Exception
	{
    ...
    protected Object checkInstanceLocation(Shell shell, Map applicationArguments) {
        ...
        if (instanceLoc.isSet()) {
            ...
            try {
                ...
                {
                	String msg = "This application appears to already be running and has a lock on the Workspace.  " +
                	...
                	boolean release = MessageDialog.openQuestion(
	                        shell,
	                        "Delete lock file?", msg);
                	if(release)
                	{
     ...
     */
    protected URL promptForWorkspace(Shell shell, ChooseWorkspaceData launchData,
			boolean force) {
        URL url = null;
        do {
	...
	{
		DARTApplicationEventDispatch.preNotify(DARTApplicationEvent.APPLICATION_STOP);
		doStop();
		DARTApplicationEventDispatch.postNotify(DARTApplicationEvent.APPLICATION_STOP);
	}
